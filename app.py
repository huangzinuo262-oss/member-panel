...
DEFAULT_USERS_ENV = os.environ.get('MEMBER_PANEL_USERS', 'admin:admin123:管理员,partner:partner123:伙伴')
...
def ensure_default_users():
    ...
    for item in DEFAULT_USERS_ENV.split(','):
        ...
        if username and password:
            create_user(username, display_name, password, role='admin')
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/logout':
            ...
            self._redirect('/login', extra_headers=[('Set-Cookie', 'member_session=; Path=/; Max-Age=0; HttpOnly; SameSite=Lax')])
            return
    ...
    def do_POST(self):
        ...
        if parsed.path == '/login':
            ...
            self._redirect('/', extra_headers=[('Set-Cookie', f'member_session={token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=2592000')])
            return
